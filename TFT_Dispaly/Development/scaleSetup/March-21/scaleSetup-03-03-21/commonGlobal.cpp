...
int8_t  WeighingHandle :: startCountComputing()
{
  ...
  initTFTHandler ( );
  printStringCOUNT( );
  showDigits.dotPosition = set._getDecimal().c_str()[0] - 48;
...
int8_t  WeighingHandle :: startWeighing()
{
  ...
  printStringWeight( );
  showDigits.dotPosition = set._getDecimal().c_str()[0] - 48;
  SPL("dotp -->> " + String( showDigits.dotPosition ) );
  ...
  while (1)
  {
    ...
    if ( handleTouchFuncationality_Weight() == -1 )
    {
      ...
      Serial.println("Serial is empty now");
      return -1;
    }
...
{
  String weightUnit = set._getWeighingUnit();
  weightStripImage();
