...
void price :: unitePrice() {
  ...
  if ( userInputAtRunTime == true ) {
...

    {
      ...
      if ( !noDotFound ) {
        leadingZero = 5 - setDot;

          switch(setDot){
          case 1: setDot = 4; break;
          case 2 :  setDot = 6 - setDot; break;
          default :  setDot = 7 - setDot; break;        
        }
     
        strcpy(showDigits.preValue[1], "ABCDEFGH");
      }
      else {
        leadingZero = 7 - setDot;
      }
...
void   price ::_rawCalculation() {
  ...
  if ( !strcmp( preAsciiValues[ PRICE_NetWeight ] , asciiValues[ PRICE_NetWeight ] ) ){
    return;
  }
