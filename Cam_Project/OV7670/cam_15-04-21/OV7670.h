...
public:
	...
	__INLINE__ void pulseLowPin(strFastPin &pin, int us) { fastDigitalWrite(pin, LOW); delayMicroseconds(us); fastDigitalWrite(pin, HIGH);  delayMicroseconds(us); }
	__INLINE__ void pulsePin(strFastPin &pin, int us) { fastDigitalWrite(pin, HIGH); delayMicroseconds(us); fastDigitalWrite(pin, LOW); delayMicroseconds(us); }
	__INLINE__ void pulsePin(strFastPin &pin) { fastDigitalWrite(pin, HIGH); fastDigitalWrite(pin, LOW); }

	__INLINE__ void FIFODiscardByte() 
	{
		fastDigitalWrite(m_fifo_rck, HIGH);
		fastDigitalWrite(m_fifo_rck, LOW);
	} // pops 1 byte off the FIFO buffer and discards

	__INLINE__ void FIFODiscardWord()
	{
		fastDigitalWrite(m_fifo_rck, HIGH);
		fastDigitalWrite(m_fifo_rck, LOW);
		fastDigitalWrite(m_fifo_rck, HIGH);
	...
	__INLINE__ uint8_t FIFOReadByte()
	{ 
		fastDigitalWrite(m_fifo_rck, HIGH);
		fastDigitalWrite(m_fifo_rck.pin, LOW);
		
		return fastDigitalRead8_1();		
	}

	 __INLINE__ uint16_t FIFOReadWord()
	{
		fastDigitalWrite(m_fifo_rck, HIGH);
		fastDigitalWrite(m_fifo_rck, LOW);
	 ...
	 __INLINE__ uint16_t FIFOReadWordRev()
	{
		fastDigitalWrite(m_fifo_rck, HIGH);
		fastDigitalWrite(m_fifo_rck, LOW);
	...
	__INLINE__ int FIFOGetClkPin() { return m_fifo_rck.pin; }
