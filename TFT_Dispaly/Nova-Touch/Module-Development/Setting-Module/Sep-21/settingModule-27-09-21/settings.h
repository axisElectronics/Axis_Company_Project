...
class settings : public userKeyBoard, public defaultSettings, public customRTC, public prtLabel
{
  private :
    ...
    typedef  struct userSettings {
      ...
      int preNode=-1;
      vector<int> connectedNode; //id of subNode >= 100
      void (*userCallBack)(void);
    };

    typedef struct s_handlePage{
  int start=0,end, totalNodeStart;
  vector<userSettings> temp;
};
  s_handlePage handlePage;
    vector <uint8_t> index;
    uint8_t dot = 0;
    vector<userSettings> userMenu, tempuserMenu;
    
  public :
void addUserMenuNode( );
void showOnTFT();
