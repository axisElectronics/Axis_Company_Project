...
#define CLEAR_INTERRUPT_BIT                                   \
    if( *int_status ) (*int_clear) |= ( 1 << GPIOPin_TIRQ );       \
    SPL("INT_READ : " + String( *int_status, HEX ) );            
...
#define ATTACH_TOUCH_INTERRUPT_Weight {                               \
    CLEAR_INTERRUPT_BIT                                        \
    attachInterrupt(digitalPinToInterrupt(GPIOPin_TIRQ), weightTouch, FALLING); \
  }
