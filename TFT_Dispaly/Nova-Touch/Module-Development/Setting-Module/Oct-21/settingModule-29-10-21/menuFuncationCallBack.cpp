...
void returnToSettingPage( class settings *settings ) {
  Enable_Text_Font()
...
int8_t saveAsDefault(  class settings *settings  ) {
  ...
  if ( temp == 1 ) {
    ...
    tft.drawString("Saving Modified Settings...", 10, 50, 1);
    tft.drawString("(", 50, 70, 1);
    tft.drawString("% )", 105, 70, 1);
     String temp = "";
    tft.drawRoundRect(50, 150, 300, 40, 10, TFT_RED);
    for (int16_t slider = 0; slider < 300; ++slider ) {      
      temp += String(slider/3);
      if( (slider%3) == 0 ){
        tft.fillRoundRect(55, 153, slider, 35, 5, TFT_GREEN);
        tft.drawString( temp.c_str(), 60, 70, 1);
      }
      temp = "";
      delay(10);
      yield();
...
int8_t TimeSetup(  class settings *settings ) {
  ...
  if ( strlen( settings->userSetting.TimeArray ) == 8 ) {
    ...
    if ( isdigit( settings->userSetting.TimeArray[0])    \
      ...
      SPL("Error 5: check Time must be digit");
      errFlag = 1;
    }
  } else {
    SPL("Error 6: check Time length");
    errFlag = 1;
  }
  ...
  if ( strlen( settings->userSetting.date ) == 8 ) {
    ...
    if ( isdigit( settings->userSetting.date[0])    \
      ...
      if ( ( settings->userSetting.date[0] >= '0' ) \
        ...
        SPL("Error 2: date must be 0..31 !!!");
        errFlag = 1;
      }
    } else {
      SPL("Error 3: check date must be digit");
      errFlag = 1;
    }
  } else {
    SPL("Error 4: check date length");
    errFlag = 1;
  }
