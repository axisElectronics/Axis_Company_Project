...
lastNodeId settings:: getLastNodeId(){
  if( userMenu.size() == 0) return 0;;
  int id = userMenu.back().id;
  return  id;
}



lastNodeId settings :: addUserMenuNode( int preNode,
                                        int nextNode,
                                        int ParentNode,
                                        int childNode,
                                        String oprName,
                                        ...
                                        int8_t (*userCallBack)( class settings *settings ) )
{
  ...
  node.id =  getLastNodeId();
  node.preNode = preNode;
  node.nextNode = nextNode;
  node.ParentNode = ParentNode;
  node.childNode = childNode;
  node.subPage = subPage;
  node.operationName = oprName;
  node.userCallBack = userCallBack;
 ...
 #define MAX_NODE 13 
    ...
    ListNode node[MAX_NODE] = {
     /* id, nextNode, preNode, ParentNode, childNode, operationName         ,  subPage,  callBackFuncation */                    
      ...
      {  4,       -1,       0,         -1,        -1, "Printer Type"        ,       NO,  mynull },
