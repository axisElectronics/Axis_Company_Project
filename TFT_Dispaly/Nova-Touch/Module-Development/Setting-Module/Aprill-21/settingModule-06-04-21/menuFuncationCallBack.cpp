  ...
  if ( strlen( settings->userSetting.TimeArray ) == 8 ) {
    ...
    if ( isdigit( settings->userSetting.TimeArray[0])    \
      ...
      if ( ( settings->userSetting.TimeArray[0] >= '0' ) \
        ...
        if ( ( settings->userSetting.date[3] > '5' ) ) {
          SPL("Error 2: minute must be 00...59 !!!" );
        }
        else
        if ( settings->userSetting.date[6] > '5' ){          
          SPL("Error 3: second must be 00...59 !!!" );
        }
        else {       
        myRTC.RTCHandler.hour    = ( settings->userSetting.date[0] - '0' ) * 10 + ( settings->userSetting.date[1] - '0' );
        myRTC.RTCHandler.minute  = ( settings->userSetting.date[3] - '0' ) * 10 + ( settings->userSetting.date[4] - '0' );
        myRTC.RTCHandler.second  = ( settings->userSetting.date[6] - '0' ) * 10 + ( settings->userSetting.date[7] - '0' );
        Serial.println("Time ===>>> " + String( myRTC.RTCHandler.hour ) + ": " + String( myRTC.RTCHandler.minute ) + ": " + String( myRTC.RTCHandler.second ) );
        myRTC.setRTCTime();
        }
      } else  SPL("Error 1: Time must be 00...12 !!!" );
    } else SPL("Error 5: check Time must be digit");
  }else SPL("Error 6: check Time length");
...
// =============================================
  ...
  Serial.println("Date : " + String(  settings->_getDate() )); // get previous set value which is stored into memory
  ...
  if ( strlen( settings->userSetting.date ) == 8 ) {
    ...
    if ( isdigit( settings->userSetting.date[0])    \
      ...
      if ( ( settings->userSetting.date[0] >= '0' ) \
        ...
        if ( ( settings->userSetting.date[3] == '1' ) \ 
        && ( settings->userSetting.date[4] > '2' ) ) {
          SPL("Error 2: month must be 1..12 !!!" );
        }
        else
        if ( settings->userSetting.date[6] < '2' ){          
        ...
        myRTC.RTCHandler.year = ( settings->userSetting.date[6] - '0' ) * 10 + ( settings->userSetting.date[7] - '0' );
        Serial.println("Date ===>>> " + String( myRTC.RTCHandler.mday ) + ": " + String( myRTC.RTCHandler.mon ) + ": " + String( myRTC.RTCHandler.year ) );
        myRTC.setRTCDate();
        }
      } else SPL("Error 2: date must be 0..31 !!!");
    } else SPL("Error 3: check date must be digit");
  }else SPL("Error 4: check date length");
