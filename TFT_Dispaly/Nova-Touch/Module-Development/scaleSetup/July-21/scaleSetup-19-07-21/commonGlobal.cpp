...
char buff[10] = "ABCDEFGH", preBuff[10] =  "ABCDEFGH", Serialtemp[10];
uint8_t  setDot, DigitAfterDot = 1;;
...
void vTask_SerialButtonMonitor( void * pvParameters ) {
  ...
  //===========================================================//
  memset(buff, '\0', 10);
  memset(priceObj.showDigits.currentValue, '\0', 10);
  ...
  for ( ;; ) {
    ...
    if (  input != -20 ) {
      ...
      else if ( input == -2 ) { // Dot has been hit by user.
        noDotFound = 0;     
      }
      else if ( noDotFound ) {
    ...
    if ( value != preValue )
    {
      ...
      if ( String(priceObj.asciiTemp).indexOf('.') < 7 ) {
        memset(priceObj.asciiValues[PRICE_perPcs], '\0', sizeof(priceObj.asciiValues[PRICE_perPcs]));
        priceObj.numericValues[PRICE_perPcs] = value;
        sprintf( priceObj.asciiValues[PRICE_perPcs], "%lf",   priceObj.numericValues[PRICE_perPcs] );
        Serial.println("Hello from task : " + String( priceObj.asciiValues[PRICE_perPcs]) );
        preValue = value ;
      }
    }
