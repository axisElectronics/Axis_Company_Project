...
class price : public WeighingHandle{
  ...
  public :
    ...
    char asciiTemp[20];
    char asciiValues[3][20];
    char preAsciiValues[3][20];
...
//    Wtft.updateLableVar( U_PRICE , asciiValues[PRICE_perPcs]);
    Wtft.updateLableVar( T_PRICE , asciiValues[PRICE_Total]);
  //  Wtft.updateLableVar( Lvar_NET , asciiValues[PRICE_NetWeight]);
...
//      SPL("ESCTouch...\n");
     Wtft.popupButton(Xesc,Ybutton, TFT_BLACK); 
      return -1;
    }
...
UNIT_PRICE:
      ...
      dtostrf(numericValues[updateField],7,2,asciiTemp_perUnit);
...
//  removeDotfromArray( showDigits.currentValue, asciiValues[PRICE_Total] );
...
//  SP("pre==>>>> "); SPL(showDigits.preValue[0]);
//  SP("current==>>>> "); SPL(showDigits.currentValue);
  //check pre fill with all Zeros
  int i;
  for(i=0; showDigits.preValue[0][i] == '0' && i < 8; ++i ); 
  if(i >= 8 ) strcpy(showDigits.preValue[0], "ABCDEFG");
...
void price :: unitePrice() {
    ...
    //1. check unit price data 
    //2. remove dot
 
    if( strlen( asciiTemp_perUnit ) ){
      for(uint8_t i=0;\
          currentValue[i];\
          currentValue[i] = asciiTemp_perUnit[i] != '.'?asciiTemp_perUnit[i++]:asciiTemp_perUnit[++i] ) yield;
           
      uint8_t len = strlen(currentValue)-1;
      currentValue[len] = '\0'; 
      
      char temp[10];
      memset(temp, '0', sizeof(temp));
      strcpy(&temp[7-strlen(currentValue)], currentValue);

      strcpy( currentValue, temp);
      
      for(uint8_t i=0; i < strlen(currentValue); currentValue[i] = isDigit(currentValue[i])? currentValue[i] : '0', ++i );
       
  //    Serial.printf("currentValue 2==>> %s\n",currentValue);
...
//        Serial.printf("leading Zeros= %d\n", leadingZero);
        ...
        Wtft.updateLableVar( U_PRICE , asciiTemp_perUnit );
}
...
void   price ::_rawCalculation() {
 ...
 // SP("ascii value : "); SPL( asciiValues[PRICE_NetWeight] );
  ...
  windowThree( showDigits.dotPosition);
