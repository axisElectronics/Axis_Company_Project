...
extern  char buff[10], preBuff[10], Serialtemp[10];
extern  uint8_t  setDot, DigitAfterDot;
extern  bool noDotFound;

void price :: unitePrice() {

 
  setDot = String(asciiValues[PRICE_perPcs]).indexOf('.');
//  SP("setDot : "); SPL(setDot);
  
  if( ( setDot >=0 && setDot < 6 ) ){
    
  for(uint8_t i = 0; i < setDot; ++i ){
    if( asciiValues[PRICE_perPcs][i] != '.'){
      buff[i] = asciiValues[PRICE_perPcs][i];
    }
  }
  buff[setDot] = '\0';
  SP("B-buff : "); SPL(buff);
  if( !noDotFound ){
  for(uint8_t i = setDot+1, j=setDot; i < setDot+3; ++i ){
    buff[j++] = asciiValues[PRICE_perPcs][i]; 
  }
  buff[setDot+3] = '\0';
  SP("A-buff : "); SPL(buff);
  DigitAfterDot = 2;
  ...
  memset(showDigits.currentValue, '0', 7 );
  strcpy( &showDigits.currentValue[7 - strlen(buff)], buff );
  }//end-setDot
  else return;
  ...
  SP("showDigits.currentValue : "); SPL(showDigits.currentValue);
     ...
     // 1. Set Font Size
  tft.setTextSize( 1 );
  tft.setFreeFont( (const GFXfont *)LED7SEG_STD18 );

  
  //6. draw Digits only,, Those digits which is different from previous Value.
  tft.setTextColor( TFT_GREEN, TFT_BLACK );
  tft.drawString(showDigits.currentValue, 30, 180, 1);
  ...
  tft.fillCircle( 41 + ( ( 6-setDot ) * 31), 216, 3, TFT_RED );
