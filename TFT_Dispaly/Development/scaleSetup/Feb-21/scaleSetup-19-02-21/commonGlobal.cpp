...
void WeighingHandle ::handleTareCommand( String temp )
{
  ...
  weightFromMachine[TARE] = strtod(cmdBuf.c_str(), NULL);
  SPL("### : " + String( weightFromMachine[TARE]) );
  {
    int8_t dotpos = 0;
    char temp[10];
    ...
    memmove( &temp[ 6 - showDigits.dotPosition - dotpos ],  weightFromMachineArray[TARE], 8);
    strcpy(weightFromMachineArray[TARE], temp );
    weightFromMachineArray[TARE][7] = '\0';
  }
  SPL("+++ : " + String( weightFromMachineArray[TARE]) );
}
