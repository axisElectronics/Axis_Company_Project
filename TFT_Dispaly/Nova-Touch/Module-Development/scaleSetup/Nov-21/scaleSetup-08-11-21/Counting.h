...
int8_t countingMachine:: activityBasedOnTouch() {
 ...
 //     SPL("1-keyboard : " + String( kbd.userInput.userInputArray ) );
      ...
      Serial.printf("1-net=%lf, unit=%lf, count=%lf\n",numericValues[COUNT_NetWeight], numericValues[COUNT_UnitWeight], numericValues[COUNT] );
      numericValues[updateField] /=  1000;
      _rawCalculation();
      Serial.printf("2-net=%lf, unit=%lf, count=%lf\n",numericValues[COUNT_NetWeight], numericValues[COUNT_UnitWeight], numericValues[COUNT] );
...
void countingMachine::  showCount() {
  ...
  else if ( ( numericValues[COUNT_UnitWeight] > 0.0 ) && ( numericValues[COUNT] == 0.0 )) {// calculate count when we have unite weight is not zero.
    //  Serial.printf("1-show numericValues net = %lf, unit = %lf\n", numericValues[COUNT_NetWeight],  numericValues[COUNT_UnitWeight] ); 
  ...
  else if ( ( numericValues[COUNT_UnitWeight] == 0.0 ) && ( numericValues[COUNT] > 0.0 ) ) {  // calculate unite weight when we have count field is not zero.
        numericValues[COUNT_UnitWeight] = (/*make netWeight into grams */( numericValues[COUNT_NetWeight] ) /  numericValues[COUNT] );
        // now update count fields
        updateField = COUNT_UnitWeight;
        memset( asciiTemp, '\0', 15);
        sprintf( asciiTemp, "%lf", numericValues[updateField]);
  ...
  else if ( ( numericValues[COUNT_UnitWeight] > 0.0 ) && ( numericValues[COUNT] > 0.0 )) {
...
void countingMachine:: onScreen(uint8_t showMeAt ) {
  ...
  switch ( showMeAt ) {
    case COUNT            : SP("Step-4 : "); SPL(showDigits.currentValue); updateTotalPcsWindow(); break;
...
void  countingMachine :: updateTotalPcsWindow() {
  ...
  sprintf( asciiValues[COUNT], "%lf",  ( round(numericValues[COUNT]/1000) ) ); 
  uint8_t tempDotPos;
  ...
  SP("Step-5 : "); SPL( showDigits.preValue[0]); 
  SP("Step-6 : "); SPL( showDigits.currentValue );   
  windowOne(-1);//disable dot and leadingZero limit
  strcpy( showDigits.preValue[0], showDigits.currentValue );
  SP("Step-7 : "); SPL( showDigits.preValue[0]);
...
void  countingMachine :: updateUnitWindow() {
  ...
  sprintf( asciiValues[COUNT_UnitWeight], "%lf",  ( numericValues[COUNT_UnitWeight]*1000 ) );
  uint8_t tempDotPos;
