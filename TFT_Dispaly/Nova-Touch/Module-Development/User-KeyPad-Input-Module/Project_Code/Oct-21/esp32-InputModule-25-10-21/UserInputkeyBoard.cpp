...
#define xLR ( ( xAxis >= x1LR ) && ( xAxis <= x2LR ) )
#define yLR ( ( yAxis >= y1LR ) && ( yAxis <= y2LR ) )
#define LRTouch (xLR && yLR )
...
#define xRR ( ( xAxis >= x1RR ) && ( xAxis <= x2RR ) )
#define yRR ( ( yAxis >= y1RR ) && ( yAxis <= y2RR ) )
#define RRTouch (xRR && yRR )
...
#define xED ( ( xAxis >= x1ED ) && ( xAxis <= x2ED ) )
#define yED ( ( yAxis >= y1ED ) && ( yAxis <= y2ED ) )
#define EDTouch (xED && yED )
...
#define findkey() zeroTouch ? zero : oneTouch ? one : twoTouch ? two : threeTouch ? three : fourTouch ? four : fiveTouch ? five :\
  ...
  delTouch ? del : entTouch ? ent : dotTouch ? dot : clrTouch ? clr : swTouch ? sw :\
  LRTouch ? Left : RRTouch ? Right : EDTouch ? edit : keepRunning;
...
int userKeyBoard ::  handleKeyPad(  ){
 /*
 * Please specify invalid character if any before calling this.
 */
  uint16_t xAxis = 0, yAxis = 0;
  int keyvalue = -20;
  int PHY_keyvalue = -20;
  ...
  // Specify invalid key into this array.
  // if not then leave it is as filled with -20(KeepRunning->defult) value.  
