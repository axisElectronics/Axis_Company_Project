...
FIXDATA :
    if ( userInputAtRunTime == true ) {
      if ( ( setDot >= 0 && setDot < 6 ) ) {
       buff = new char[10];
        ...
        for (uint8_t i = 0; i < setDot; ++i ) {
          ...
          buff[setDot + 0] = asciiValues[PRICE_perPcs][setDot + 1];
          buff[setDot + 1] = asciiValues[PRICE_perPcs][setDot + 2];
          buff[setDot + 2] = '\0';
          if ( DigitAfterDot == 2 ) {
        ...
        value = 0;
        memset(asciiValues[PRICE_perPcs], '0', 7);
        memset(showDigits.currentValue, '0', 7 );
        goto EXIT;
      }
...

      {
        ...
        //1. check with previous Value
        if ( !strcmp( showDigits.preValue[1] , showDigits.currentValue ) ) {
          goto EXIT;
        }
