...
static TaskHandle_t xHandle_Touch;
//static uint16_t xAxis, yAxis;



void vTask_TouchMonitor( void * pvParameters ) {

for(;;){
     esp_task_wdt_reset();
    pressed = tft.getTouch( &xAxis, &yAxis, 20);
    if( pressed ){
      SP("pressed : "); SPL(pressed);
    }
...
static void createThread_Touch() {

  xTaskCreate(   vTask_TouchMonitor,
                 "Touch Monitor",
                 8*1048, // if get any issue please increase stack size to 4096
                 NULL,
                 2,
                 &xHandle_Touch );
}
