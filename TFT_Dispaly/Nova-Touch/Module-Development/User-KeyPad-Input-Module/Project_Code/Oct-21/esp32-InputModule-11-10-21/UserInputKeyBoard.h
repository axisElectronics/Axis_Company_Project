...
class userKeyBoard: public  SEPL328_customKeypad
{
  ...
  public :
     virtual void moveUrDash( int8_t idx );
    ...
    struct _userInput{
      ...
      bool switchFlag;            // switch flag which emphasis user to take input in specified format.
      ...
      char unitbuf[10]; // it will show unit of every keypad input values. 
    ...
    virtual String takeStringInput(char *ptrbuff, int len);
   ...
   virtual  uint8_t step_default(  char temp, uint8_t inc    );
   virtual  uint8_t step_clr    (  char temp, uint8_t inc    );
   virtual  uint8_t step_del    (  char temp, uint8_t inc    );
   virtual  void    step_back   (  char temp, uint8_t inc    );
   virtual  void    step_sw     (  char temp, uint8_t inc    );
