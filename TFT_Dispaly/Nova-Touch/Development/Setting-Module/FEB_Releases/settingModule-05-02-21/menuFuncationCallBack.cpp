...
void showDigitfun(struct _showDigit *showDigit, int yPos )
{
  ...
  SPL("len : " + String(len) );
  SPL("temp : " + String(showDigit->currentValue) );
  ...
  // 4. print those digit only which is not same as previous.
  for ( ; idx < 8; ++idx )
  {
...
void readNoLoadcount( struct s_setting *basesetting )
{
  ...
  //  memset(showDigit.currentValue, "0", 7);
//  showDigit.currentValue[0] = '0';
  ...
  while ( caliButtonTouch() != NOLOAD )
  {
    if ( Serial2.available() > 5 )
    {
      ...
      { strcpy( &showDigit.currentValue[1], noLoad_count.c_str() );
        showDigit.currentValue[strlen( showDigit.currentValue) - 2] = '\0';
        showDigitfun( &showDigit, 65 );
      }
    }
  ...
  memset(showDigit.currentValue, 'A', strlen( showDigit.currentValue) );
  memset(showDigit.preValue, 'A', strlen( showDigit.preValue) );
...
void readLoadcount( struct s_setting *basesetting )
{
  ...
  memset(showDigit.currentValue, '0', 10);
  while ( caliButtonTouch() != LOAD )
  {
    if ( Serial2.available() > 5 )
    {
      ...
      strcpy( &showDigit.currentValue[1], load_count.c_str() );
      showDigit.currentValue[strlen( showDigit.currentValue) - 2] = '\0';
      showDigitfun( &showDigit, 140 );
    }
  ...
  memset(showDigit.preValue, '0', strlen( showDigit.preValue) );
  strcpy( &showDigit.currentValue[1], basesetting->userSetting->noLoad_count );
  showDigitfun( &showDigit, 65 );
  memset(showDigit.preValue, '0', strlen( showDigit.preValue) );
  strcpy( &showDigit.currentValue[1], basesetting->userSetting->load_count );
  showDigitfun( &showDigit, 140 );
  memset(showDigit.preValue, '0', strlen( showDigit.preValue) );
  memset(showDigit.currentValue, '0', strlen( showDigit.preValue) );
  strcpy( &showDigit.currentValue[1], basesetting->userSetting->cali_weight );
  showDigitfun( &showDigit, 215 );
    ...
    if ( caliButtonTouch() == CALI_WEIGHT )
    {
      ...
      while ( caliButtonTouch() != SUBMIT )
      {
        if ( Serial2.available() > 5 )
        {
          ...
          strcpy( showDigit.currentValue, CalibratedWeight.c_str() );
          showDigit.currentValue[strlen( showDigit.currentValue) - 2] = '\0';
          showDigitfun( &showDigit, 285 );
        }
