...
void   checkWeighing :: onScreen(uint8_t showMeAt ) {
  ...
  switch ( showMeAt ) {
    case CHECK_NetWeight :   windowOne(showDigits.dotPosition); break;
    case MAX             :   windowTwo(showDigits.dotPosition); break;
    case MIN             :   windowThree(showDigits.dotPosition); break;
  }//end-switch
}//end-onScreen()
...
void  checkWeighing :: _start() {
  ...
  // read previous capacity from file system
    String temp = _getCapacity();
    strcpy( asciiTemp, temp.c_str() );
    numericValues[updateField] = strtod(asciiTemp, NULL)/1000;
    dtostrf(numericValues[updateField],7, showDigits.dotPosition,asciiTemp);
   // Serial.printf("numericValues[MAX]=%lf, asciiTemp=%s\n", numericValues[updateField], asciiTemp);
