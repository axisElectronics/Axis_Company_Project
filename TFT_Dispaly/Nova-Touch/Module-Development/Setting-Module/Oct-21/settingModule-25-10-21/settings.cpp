...
struct su_settingMenu settingMenuArray[50] = {
  ...
  /* 7 */ { 17, "Factory Default"            , 0         , factoryDefault/* This function define cordingly */ },
  /* 8 */ { 18, "Service"                    , 0        , SecurityTest/* This function define cordingly */ },
  ...
  /* 4.4 */ { 143, "Product"                 , 0         , addProductName/* This function define cordingly */ },
  /* 4.5 */ { 144, "Operator"                , 0         , addOperatorName/* This function define cordingly */ },
  /* 4.6 */ { 145, "Shift"                   , 0         , addShift/* This function define cordingly */ },
...
class DisplayInit distft;
  distft.RpiDisplayInit(typeTwo);

  basesetting.baseStringIdx = basesetting.stringIdx = 10;
  basesetting.baseEndStringIdx = 20;

  // go into Settings and use factoryReset option. To activate this[deprecate in new revisions...]
  // initDefaultSettings( );
...
{
  // Let User Choose option Wheather He is Admin or Regular User
    // 1. Regulat User
  tft.fillScreen(TFT_CYAN);
  Enable_Text_Font()
  tft.setTextColor(TFT_GREEN, TFT_BLACK);
//  tft.fillRoundRect(int16_t x0, int16_t y0, int16_t w, int16_t h, int16_t radius, uint16_t color);
  tft.fillRoundRect(20, 100, 140, 100, 10, TFT_BLACK);
  tft.drawString("Admin", 30, 110);
  tft.fillRoundRect(220, 100, 140, 100, 10, TFT_BLACK);
  tft.drawString("User", 250, 110);
  tft.fillRoundRect(20, 245, 120, 60, 10, TFT_BLACK);
  tft.drawString("Back", 23, 252);
  ...
  for(int8_t i=0; i < 15; ++i ){
   tft.drawCircle(200, 200, 10+i, TFT_RED);
    delay(50);
    yield();
  }
  
for(int8_t i=14; i >= 0; --i ){
   tft.drawCircle(200, 200, 10+i, TFT_BLACK);
    delay(50);
    yield();
  }
  delay(100);
