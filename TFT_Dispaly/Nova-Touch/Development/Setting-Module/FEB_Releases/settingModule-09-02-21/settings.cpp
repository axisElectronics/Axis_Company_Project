...
void settingPageInit( struct s_setting *basesetting, struct su_settingMenu *settingMenuArray, class settings *classOBJ_set)
{
  ...
  // defualt settings save permantly in EEPROM
   basesetting->classOBJ_dftsett = &dftsett; 

// basesetting->classOBJ_dftsett->_setWeightUnit(2);
 
 Serial.println("_getWeightUnit : " + String( basesetting->classOBJ_dftsett->_getWeightUnit() ) );
   
  // structure settings
  basesetting->userSetting = &nova_setting;
  basesetting->showDigit = &showDigit;
