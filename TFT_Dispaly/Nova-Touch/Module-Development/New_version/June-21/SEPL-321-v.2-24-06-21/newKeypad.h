...
class GenericKeypad {
  ...
  public :
    ...
    int8_t getInput();
    void enableInterrupt( void (*UserISR_fptr)(void), int8_t interruptPin );
...
void GenericKeypad :: enableInterrupt( void (*UserISR_fptr)(void), int8_t interruptPin ) {
  ...
  if ( UserISR_fptr )
    attachInterrupt(digitalPinToInterrupt(_interruptPin), UserISR_fptr, FALLING);
  ISR_Called = 0;
}
