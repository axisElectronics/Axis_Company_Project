...
// Leave it for some time as it is and wait for NEERAJ Sir's
// firmware update.
// As we discussed on 16-10-2021, Jagdeep Sir have Concerns about handling of Errors on esp32.
// He does not want to have control at two MCUs.
// He instructed that Leave error handling to Neeraj Sir's Firmware because it is 
// mature enough over the period of time.
// My task is to provide only UI part and minimum calculation which is non-avoidable.
...
void weight:: _start() {
  ...
  if( showDigits.dotPosition > 6 ){
    showDigits.dotPosition = 3;
    _setDecimal('3');
  }
