 ...
 *****************************************************************************/
extern TaskHandle_t xPriceHandler;
TaskHandle_t xwindowHandler = NULL ;
char buff[8];
 
void unitePriceTemp( void * taskbuff ){
...
for(;;){
  ...
  SP("unitePriceTemp : ");SPL((char *)buff); 
//  vTaskSuspend( &xPriceHandler );
  // 1. Set Font Size
  Wtft.showDigits.digitFontSize  = 1;
  Wtft.showDigits.digitFontStyle = LED7SEG_STD18;
  tft.setTextSize(  Wtft.showDigits.digitFontSize );
  tft.setFreeFont( (const GFXfont *) Wtft.showDigits.digitFontStyle );

  yield();
  //2. draw blank rectangle and background Digits
  ...
  for (uint8_t idx = 0; idx < 7; ++idx )
  {
   // if ( showDigits.preValue[1][idx] !=  showDigits.currentValue[idx] )
    {
      tft.drawChar(  buff[idx], 15 + ( idx * 31) , 210,  1);
    }
     yield();
  }
 // vTaskResume( &xPriceHandler ); 
...
// init Threads funcations
void createPriceThread(){
...
// esp_task_wdt_add( &xPriceHandler ); //add current thread to WDT watch               
   ...
   if(  input != -20 ){  
    ...
    while( ( millis() - Timeout ) < 2000  ){      
       if(  input != -20 ){
        ...
        if( input == -2 ){// Dot has been hit by user.
          noDotFound = 0;
          Timeout = millis();
        }
        else if( noDotFound ){
         ...
         // create a new task
        // show Input on window-2
        xTaskCreate(  unitePriceTemp, 
                  "Price Input", 
                  10000, 
                  buff, 
                  0, 
...
//         unitePriceTemp(buff);
   ...
   if( xwindowHandler != NULL )
      vTaskDelete( &xwindowHandler );
   
   if( value != 0.0 )
   {
    memset(priceObj.asciiTemp, '\0', sizeof(priceObj.asciiTemp));   
    sprintf( priceObj.asciiTemp, "%lf",  value );
    Serial.println("Hello from task : " + String(priceObj.asciiTemp) );
    priceObj.updateField = PRICE_perPcs; 
    priceObj._rawCalculation(); 
   }
...
//  if( i < ( dotpos ) )  showDigits.currentValue[i] = '0';
//  else{ showDigits.currentValue[i] = temp.c_str()[ i - dotpos ];
// }
//
