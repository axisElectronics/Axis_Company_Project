...
void  countingMachine :: updateUnitWindow() {
  ...
  if ( tempDotPos < 7 ) {
    if ( ( tempDotPos == 1 ) && (asciiValues[COUNT_UnitWeight][0] == '0') ){
    // tempArray have data without dot.
    memset(tempArray, '\0', 15);
    memset(tempArray, '0', tempDotPos);
    strcat(tempArray, &asciiValues[COUNT_UnitWeight][tempDotPos + 1]);
    }
    else{
      // tempArray have data without dot.
    memset(tempArray, '\0', 15);
    for(uint8_t i=0; i < tempDotPos; tempArray[i] = asciiValues[COUNT_UnitWeight][i++] );
    strcat(tempArray, &asciiValues[COUNT_UnitWeight][tempDotPos + 1]);
    }
  } else return;
...
/*
  for(int8_t i = 6; showDigits.currentValue[i] != '0'; --i) leadingZero++;
  memset(tempArray, '0', 7);
  for( uint8_t i = leadingZero; i < 7; ++i ){
    tempArray[i] = showDigits.currentValue[i-3];
  }
 strcpy( showDigits.currentValue,  tempArray );
 showDigits.currentValue[7] = '\0';
...
/*
  ...
  tft.setTextColor( makeCustomColor(10, 10, 10), TFT_BLACK );
  for (uint8_t idx = 0; idx < leadingZero-1; ++idx )
  {
...
*/
  ...
  int8_t dotPosition = tempDotPos-1;
