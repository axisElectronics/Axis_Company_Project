...
void defaultSettings :: initEEPROM()
{
...
void  defaultSettings ::  setDefaultSettings()
{
  // 1. initialise EEPROM
  initEEPROM();
...
int8_t  defaultSettings :: _setBuadrate(char *baudrate)
{
  int8_t temp;
...
String defaultSettings ::  _getBuadrate()
{
...
int8_t  defaultSettings :: _getDataBits()
{
...
int8_t   defaultSettings :: _setDataBits(int8_t DataBits )
{
  int8_t temp;
  if ( ( DataBits == 8 ) )
  {
...
int8_t defaultSettings ::  _getStopBits()
{
...
int8_t  defaultSettings ::  _setStopBits(int8_t StopBits )
{
...
String  defaultSettings :: _getParityBits()
{
...
int8_t defaultSettings ::   _setParityBits(char *ParityBits )
{
  int8_t temp = 0;
...
char * defaultSettings :: _setDate(char *date )
{
  int8_t temp = 0;
...
int8_t  defaultSettings :: _geteValue()
{
  int8_t temp;
...
int8_t  defaultSettings :: _seteValue( int8_t eValue )
{
  EEPROMWrite( eValueAddress , eValue  );
  return byte( EEPROM.read( eValueAddress ) );
}

int8_t  defaultSettings :: _getdecimal()
{
  int8_t temp;
...
int8_t  defaultSettings :: _setdecimal(int8_t decimal )
{
...
char * defaultSettings :: _setCapacity( char *capacity )
{
...
String  defaultSettings :: _getWeightUnit()
{
...
// initialize EEPROM with predefined size
  ...
  int8_t unit = byte( EEPROM.read( weightingUnitsAddress ) );
  unit = (unit & 0xF0)>> 4;
  String tempBuf = "";
  ...
  switch (unit)
  {
    case 0: tempBuf += "kg"; break;
    case 1: tempBuf += "g"; break;
    case 2: tempBuf += "t"; break;
    case 3: tempBuf += "oz"; break;
    case 4: tempBuf += "lb"; break;
    default: break;
  }
  return tempBuf;
}
...
int8_t defaultSettings ::  _setWeightUnit( int8_t weightUnit )
{
  int8_t temp = ( byte( EEPROM.read( weightingUnitsAddress ) & 0x0F ) | ( 0xF0 & ( weightUnit << 4) ) );
 
  EEPROMWrite( weightingUnitsAddress , temp );
  return ( byte( EEPROM.read( weightingUnitsAddress ) ) & (0xF0) )>>4;
}


String  defaultSettings :: _getApplication()
{
  int8_t application = ( byte( EEPROM.read( weightingUnitsAddress ) ) & 0x0F );
  String tempBuf = "";
  switch (application)
  {
    case 0: tempBuf += "Weighing Mode"; break;
    case 1: tempBuf += "Counting Mode"; break;
    case 2: tempBuf += "Check Weighing "; break;
    case 3: tempBuf += "Price Computing"; break;
  }
  return tempBuf;
}
...
{
  application = application & 0x0F;
  int8_t temp = ( byte( EEPROM.read( weightingUnitsAddress ) ) & ( 0xF0 | application ) );
  
  EEPROMWrite( weightingUnitsAddress , temp );
  return ( byte( EEPROM.read( weightingUnitsAddress ) ) & (0x0F) );
}
