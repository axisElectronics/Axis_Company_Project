...
int _getCharacter( int keyvalue, bool switchFlag, struct _userInput *userInput )
{
  ...
  if( userInput->fixFormatstruct->fixFormatFlag )
  {
  ...
  if ( tft.getTouch(&xAxis, &yAxis) )
  {
    ...
    switch ( keyvalue )
    {
      ...
      case nine:
        ...
        return _getCharacter( keyvalue, switchFlag, userInput );
    }//end-switch
  }
...
{
   /* It is mandatory to enable fixformatFlag otherwise developer not get desired output */
  strcpy( userInput->fixFormatstruct->fixFormatArray, "DD/MM/YY" );//
  userInput->fixFormatstruct->fixFormatArraySize = strlen( userInput->fixFormatstruct->fixFormatArray );
  userInput->fixFormatstruct->rollOverChar = '/'; // It can be multiple chacter developer want to rollOver in Fix Format.
//  userInput->fixFormatFlag = 1;

/* Developer has only 17 character size to print fix format. */
  fixFormatFullBlankScreen

  for(int8_t i = 0; i < userInput->fixFormatstruct->fixFormatArraySize; i++ )
  {
...
//      userInput->userCustomKeypad( userInput );
  ...
  while ( 1 )
  {
    ...
    switch ( temp )
    {
      ...
      case del :
        if( userInput->fixFormatstruct->fixFormatFlag && (  userInput->fixFormatstruct->idx  < userInput->fixFormatstruct->fixFormatArraySize ) )
          {             
             if(  userInput->fixFormatstruct->idx <= 0 )  userInput->fixFormatstruct->idx = 0;
             else if(  userInput->fixFormatstruct->idx > 0)
              moveUrDash( --userInput->fixFormatstruct->idx, userInput );

            if( userInput->fixFormatstruct->rollOverChar == userInput->fixFormatstruct->fixFormatArray[ userInput->fixFormatstruct->idx ] ) moveUrDash( --userInput->fixFormatstruct->idx, userInput );           
              delay(300);
          }
        else if( !userInput->fixFormatstruct->fixFormatFlag )
        {
      ...
      default :
        if ( userInput->entIdx <  userInput->userInputArray_Size )
        {
          ...
          if( userInput->fixFormatstruct->fixFormatFlag && (  userInput->fixFormatstruct->idx  < userInput->fixFormatstruct->fixFormatArraySize-1 ) )
          {  
