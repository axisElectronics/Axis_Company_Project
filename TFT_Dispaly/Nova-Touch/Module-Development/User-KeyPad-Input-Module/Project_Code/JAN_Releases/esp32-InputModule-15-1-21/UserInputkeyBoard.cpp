...
void  userKeyBoard :: takeUserInput( struct _userInput *userInput )
{
  uint8_t inc = 0;
...
void userKeyBoard ::  step_clr( struct _userInput *userInput, char temp, uint8_t inc )
{
  if ( userInput->fixFormatstruct && userInput->fixFormatstruct->fixFormatFlag )
  {
    switch ( userInput->fixFormatstruct->fieldType )
...
void userKeyBoard ::  step_del( struct _userInput *userInput, char temp, uint8_t inc )
{
      ...
      switch ( userInput->fixFormatstruct->fieldType)
      {
        ...
        {    
          --userInput->fixFormatstruct->idx;    
           Serial.println("del : " + String(userInput->fixFormatstruct->idx) );
  ...
  else if (  !( userInput->fixFormatstruct && userInput->fixFormatstruct->fixFormatFlag ) )
  {
    if ( userInput->entIdx <  userInput->userInputArray_Size )
    {
      ...
      {       
        ...
        if ( userInput->entIdx == userInput->userInputArray_Size - 1)  userInput->userInputArray[ userInput->entIdx-1 ] = '\0';
        Serial.println("--->>>## " + String( userInput->userInputArray ) );
        tft.drawString( &userInput->userInputArray[++inc], userInput->text_xAxis, userInput->text_yAxis, 1);
      }
    }
