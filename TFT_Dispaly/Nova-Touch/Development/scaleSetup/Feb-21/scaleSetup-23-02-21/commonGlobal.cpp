...
void  WeighingHandle :: handleTouchFuncationality()
{
...
void  WeighingHandle :: handleTouchFuncationality_PRICE()
{
  uint16_t xAxis = 0, yAxis = 0;
  char src[12] = {0};
  if ( tft.getTouch(&xAxis, &yAxis) )
  {
    ...
    else if ( Field_three_Touch  )
    {
      ...
      strcpy( showDigits.preValue[1], "ABCDEFGH" );
      strcpy( showDigits.preValue[2], "ABCDEFGH" );
//      SPL("dotposENTER --- >>> " + String( showDigits.dotPosition) );
      _updateWindowPricing(perPCS);
//      SPL("dotposExit --- >>> " + String( showDigits.dotPosition) );
    }
  }
}


void  WeighingHandle :: startPriceComputing()
{
  char src[12];
  char dest[10];
  ...
  _updateWeightperPrice( "5.00" );

//  SPL("dotpos : " + String( showDigits.dotPosition ) );
  _updateWindowPricing(perPCS);
// SPL("dotpos : " + String( showDigits.dotPosition ) );
...
void  WeighingHandle :: startWeighing()
{
  char src[12];
  char dest[10];
...
//      SPL("_updateWindowPricingENTERY : " + String( showDigits.dotPosition ) ); 
      windowThree( ); 
//      SPL("_updateWindowPricingExit : " + String( showDigits.dotPosition ) );    
      break;
  }
...
bool  WeighingHandle :: printString( )
{
